...
simdat <- calibrator |>
  mp_optimized_spec("modified") |>
  mp_simulator(time_steps, outputs = c("double_vac", "booster_shot", "S1","V2","V3","E1","E2","E3","A1","A2","A3","I1","I2","I3","R1","R2","R3")) |> 
  mp_trajectory() |>
  lim_dat() |>
...
library(scales)
# load reported data
reported_cases <- read.csv("~/quantifying_omicron_burden_in_NL/data/serop_avgcase_data.csv")
...
reported_cases <- reported_cases |>
  drop_na() #|>
  # dplyr::filter(date >= "2022-01-31")

print(max(reported_cases$cases, na.rm = TRUE))
