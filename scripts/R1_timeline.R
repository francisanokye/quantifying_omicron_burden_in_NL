...
p <- ggplot() +
  ...
  geom_segment(data = axis_dates, aes(x = date, xend = date, y = y_axis + 0.02, yend = y_axis)) +
  geom_text(data = axis_dates, aes(x = date, y = y_axis + 0.05, label = label), size = 8) +
  ...
  geom_text(data = events_als, aes(x = date, y = y_txt, label = label), size = 7) +
  
  # als band with shading
  geom_rect(data = als_segments,
            aes(xmin = start, xmax = end, ymin = y_als - h_als, ymax = y_als + h_als, fill = level),
            color = "black") +
  geom_text(data = als_segments, aes(x = midpoint, y = y_als, label = level), size = 8, face = "bold") +
  ...
  geom_segment(data = k12_verticals,
               ...
               linetype = "dashed", color = "red", size = 2.0) +
  ...
  geom_text(data = schools_closed,
            ...
            size = 8, color = "blue", vjust = 0.2, hjust = 0.5) +
  ...
  geom_rect(data = testing_segments,
            ...
            fill = NA, color = "black") +
  geom_text(data = testing_segments, aes(x = midpoint, y = y_test, label = label), size = 8) +
...
png("../figures/R1_Figure_1.png", width = 5000, height = 2500, res = 300, bg = "white", type = "cairo")
print(p)
dev.off()